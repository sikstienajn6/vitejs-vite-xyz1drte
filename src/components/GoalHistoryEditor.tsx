    ...
    <div className="mt-8">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          type="button"
          onClick={handleAdd}
          className="flex items-center gap-1 text-xs font-bold text-blue-400 hover:text-blue-300 bg-blue-500/10 px-2 py-1.5 rounded-lg"
        >
      ...
      <div className="space-y-3">
        {sortedPeriods.slice().reverse().map((p) => {
          ...
          if (isEditing) {
            return (
              <div key={p.id} className="bg-slate-800 p-4 rounded-xl border border-slate-700 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] uppercase font-bold text-slate-500">Start Date</label>
                    <input type="date" value={editStartDate} onChange={e => setEditStartDate(e.target.value)} className="w-full bg-slate-900 border border-slate-700 text-white rounded-lg px-2 py-1.5 text-sm" />
                  </div>
                  <div>
                    <label className="text-[10px] uppercase font-bold text-slate-500">End Date</label>
                    <input type="date" value={editEndDate} onChange={e => setEditEndDate(e.target.value)} disabled={isOpen} className="w-full bg-slate-900 border border-slate-700 text-slate-400 rounded-lg px-2 py-1.5 text-sm disabled:opacity-50" />
                  </div>
                </div>
                <div>
                ...
                <div className="flex gap-2 pt-2">
                  <button type="button" onClick={() => handleSaveEdit(originalIdx)} className="flex-1 bg-emerald-500 text-white font-bold py-1.5 rounded-lg text-sm">Save</button>
                  <button type="button" onClick={handleCancelEdit} className="flex-1 bg-slate-700 text-white font-bold py-1.5 rounded-lg text-sm">Cancel</button>
                </div>
              </div>
            );
          }
          ...
          return (
            <div key={p.id} className="flex items-center justify-between bg-slate-800/50 p-3 rounded-xl border border-slate-700/50">
              ...
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => handleEditInit(p)} className="p-1.5 text-slate-500 hover:text-blue-400 bg-slate-800 rounded-lg"><Edit2 size={14} /></button>
                <button type="button" onClick={() => handleDelete(originalIdx)} className="p-1.5 text-slate-500 hover:text-red-400 bg-slate-800 rounded-lg"><Trash2 size={14} /></button>
              </div>
            </div>
          );
        })}
